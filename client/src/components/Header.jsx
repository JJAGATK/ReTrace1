      ...
      <div className="max-w-[1240px] mx-auto px-4 sm:px-6 lg:px-8 h-16 md:h-20 flex items-center justify-between gap-3 md:gap-6">
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          <div className="relative">
            ...
            {showNotifications && (
              <div className="absolute right-0 mt-2 w-80 rounded-2xl glass-card p-3 shadow-2xl border border-indigo-200/80 z-50 animate-in fade-in zoom-in-95 duration-150">
                <div className="flex items-center justify-between pb-2 border-b border-indigo-100/80 mb-2">
                  <div className="flex items-center gap-1.5">
                    <span className="material-symbols-outlined text-sm text-[#4648d4]">notifications_active</span>
                    <span className="text-xs font-bold text-[#1a1b25]">Campus Recovery Alerts</span>
                  </div>
                  <span className="text-[10px] font-semibold text-emerald-700 bg-emerald-50 px-1.5 py-0.2 rounded-full">Live</span>
                </div>
                ...
                <div className="flex flex-col gap-1.5">
                  {notifications.map((n) => (
                    <div
                      ...
                      className="p-2 rounded-xl hover:bg-indigo-50/80 transition-colors cursor-pointer flex items-start gap-2.5"
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between">
                          <span className="text-xs font-bold text-[#1a1b25] truncate">{n.title}</span>
                          <span className="text-[9px] text-slate-400 shrink-0">{n.time}</span>
                        </div>
                        <p className="text-[11px] text-[#464554] line-clamp-1 mt-0.5">{n.desc}</p>
                      </div>
                    </div>
                  ))}
                </div>

                <div className="pt-2 border-t border-indigo-100/80 mt-1">
                  <button
                    onClick={() => {
                      setShowNotifications(false);
                      setCurrentTab('handover');
                    }}
                    className="w-full py-1.5 text-center text-xs font-semibold text-[#4648d4] hover:bg-indigo-50 rounded-lg transition-colors cursor-pointer"
                  >
