...
export default function ItemDetailScreen({ item, onBack, onClaimSuccess, onNavigate }) {
  ...
  return (
    <div className="relative z-10 max-w-[1240px] mx-auto px-4 sm:px-6 lg:px-8 pt-4 md:pt-6 pb-24">
      ...
      {showZoomModal && (
        <div className="fixed inset-0 z-50 bg-[#1a1b25]/90 backdrop-blur-md flex flex-col items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="relative max-w-4xl w-full flex flex-col items-center gap-4">
            <button
              onClick={() => setShowZoomModal(false)}
              className="absolute -top-10 right-0 text-white hover:text-slate-300 p-2 rounded-full cursor-pointer flex items-center gap-1 text-xs"
            >
              <span className="material-symbols-outlined text-2xl">close</span>
            </button>
            <div className="max-h-[75vh] w-full flex items-center justify-center overflow-hidden rounded-2xl bg-black/40 border border-white/20 p-2">
              <img
                src={photos[activePhotoIdx]}
                alt={item.title}
                ...
                {photos.map((p, i) => (
                  <button
                    ...
                    className={`w-14 h-14 rounded-lg overflow-hidden border-2 cursor-pointer ${
                      activePhotoIdx === i ? 'border-white scale-105' : 'border-transparent opacity-60'
                    }`}
                  >
      ...
      {showSightingModal && (
        <div className="fixed inset-0 z-50 bg-[#1a1b25]/60 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="w-full max-w-md bg-white rounded-3xl p-5 sm:p-6 shadow-2xl border border-indigo-200/80 flex flex-col gap-4">
        ...
        <div className="fixed inset-0 z-50 bg-[#1a1b25]/60 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="w-full max-w-lg bg-white rounded-3xl p-5 sm:p-6 shadow-2xl border border-indigo-200/80 flex flex-col gap-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex justify-end pt-2">
              <button
                onClick={() => setShowCustodyModal(false)}
                className="px-5 py-2 rounded-full bg-slate-100 hover:bg-slate-200 text-[#1a1b25] text-xs font-semibold cursor-pointer"
              >
                Close Audit Log
              </button>
            </div>
