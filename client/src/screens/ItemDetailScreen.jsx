...
export default function ItemDetailScreen({ item, onBack, onClaimSuccess }) {
  ...
  const handleClaimSubmit = async (e) => {
    ...
    setClaimFeedback('Evaluating ownership challenge with secure campus engine...');
    ...
    <div className="relative z-10 max-w-[1240px] mx-auto px-4 sm:px-6 lg:px-8 pt-4 md:pt-6 pb-24">
        ...
        <section className="lg:col-span-7 flex flex-col gap-4">
          ...
          <div className="glass-panel rounded-2xl md:rounded-3xl p-3 sm:p-4 shadow-md flex flex-col gap-3 border border-indigo-200/50">
            <div className="relative w-full aspect-[4/3] rounded-xl md:rounded-2xl overflow-hidden bg-indigo-50/50 group">
              ...
              <div className="absolute bottom-3 right-3 pointer-events-auto">
                <button
                  onClick={() => showToast('Full high-resolution proof inspected')}
                  className="flex items-center gap-1 px-3 py-1.5 rounded-full bg-[#1a1b25]/85 hover:bg-[#1a1b25] text-white backdrop-blur-md text-xs font-semibold shadow-xs cursor-pointer"
                >
          ...
          <div className="glass-panel rounded-2xl md:rounded-3xl p-4 sm:p-5 flex flex-col gap-3 border border-indigo-200/50">
            <div className="flex items-center justify-between flex-wrap gap-2">
              ...
              <button
                onClick={() => showToast('Campus geofenced map coordinates selected')}
                className="inline-flex items-center gap-1 px-3 py-1.5 rounded-full bg-indigo-50 hover:bg-indigo-100 text-[#4648d4] font-semibold text-xs border border-indigo-200/70 cursor-pointer"
              >
            ...
            <div className="relative w-full h-44 rounded-xl overflow-hidden bg-[#eeecfc] border border-indigo-200/60 flex items-center justify-center">
              ...
              <div className="absolute bottom-2.5 left-2.5 px-2.5 py-1 rounded-full bg-white/90 backdrop-blur-md shadow-xs flex items-center gap-1.5 text-[11px] font-medium text-[#1a1b25] border border-indigo-100">
                ...
                <span>Coarse Zone (Exact Coordinates Protected)</span>
              </div>
            </div>
          </div>
        ...
        <section className="lg:col-span-5 flex flex-col gap-4">
          <div className="glass-panel rounded-2xl md:rounded-3xl p-5 sm:p-6 flex flex-col gap-4 border border-indigo-200/60 shadow-lg shadow-indigo-500/5">
            ...
            <div className="p-3.5 rounded-2xl bg-white/70 border border-indigo-100 flex items-center justify-between gap-3">
              ...
              <button
                onClick={() => showToast('Encrypted dispatch note sent to desk officer')}
            ...
            <div className="flex flex-col gap-2 pt-1">
              {item.type === 'found' && item.status !== 'returned' ? (
                ...
                <button
                  type="button"
                  onClick={() => showToast('Your clue was recorded and sent to the owner!')}
              ...
              <div className="grid grid-cols-3 gap-2 pt-1">
                <button
                  onClick={() => showToast('Cabot Desk dispatch connected')}
                ...
                <button
                  onClick={() => showToast('Flag report logged for campus moderator review')}
      ...
      {showClaimModal && (
        <div className="fixed inset-0 z-50 bg-[#1a1b25]/60 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="w-full max-w-lg bg-white rounded-3xl p-5 sm:p-6 shadow-2xl border border-indigo-200/80 flex flex-col gap-4 max-h-[90vh] overflow-y-auto">
